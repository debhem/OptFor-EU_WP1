###################################################################################################################################################
# Title: Script for Concatenating and Regridding Future Climate Scenarios of EURO-CORDEX Minimum air temperature data

# Date: 20th January 2025

# Author: Dr Deborah Hemming and Dr Murk Memon, Met Office Hadley Centre, Met Office, UK

# Description: Code to concatenate (if needed) and regrid monthly mean future projection data from EURO-CORDEX regional climate models (RCM) for the variable "minimum air temperature"
...
#         EURO-CORDEX RCM climate projection data for monthly mean maximum air temperature
#            - Downloaded as 'Minimum 2m temperature in the last 24 hours' in K, see https://cds.climate.copernicus.eu/datasets/projections-cordex-domains-single-levels?tab=overview
#            - With the naming format (for data in 5-year chunks) is "/path/to/EURO-CORDEX/RCP26/HIRHAM5/mintair_mon/mintair_europe_[MODEL]_[SCENARIO]_mon_2006_2100.nc" 
#                 - Or if time series are already concatenated the naming format is "/path/to/EURO-CORDEX/RCP26/HIRHAM5/concat/mintair_europe_[MODEL]_[SCENARIO]_mon_2006_2100.nc" 
#            - For 2 RCM - HIRHAM5 and RACMO22E
#            - And 3 future scenarios - RCP26, RCP45, RCP85

# Outputs: NetCDF time series files (2006-2100) for the variable "miinimum air temperature" 
#          With the naming format "/path/to/EURO-CORDEX/RCP26/HIRHAM5/concat/mintair_europe_[MODEL]_[SCENARIO]_mon_2006_2100.nc" 
...
# *** If needed *** If data have been downloaded from CDS it will be in 5-year time chunks, these will need concatenating into a single NetCDF file
   ...
   ## files = /path/to/EURO-CORDEX/RCP85/HIRHAM5/mintair_mon/
...
def main():
   # Location of the input EURO-CORDEX HIRHAM5 minimum air temperature data file/s
   ...
   mintair_input = '/path/to/EURO-CORDEX/RCP26/HIRHAM5/mintair_mon/mintair_mon.nc' # Change filenames to correct RCP and MODEL

   # Location of the processed, output EURO-CORDEX HIRHAM5 minimum air temperature NetCDF data file
   mintair_output = '/path/to/EURO-CORDEX/RCP26/HIRHAM5/concat/mintair_europe_HIRHAM5_RCP26_mon_2006_2100.nc'
   ...
   latlong_grid = '/path/to/ERA5_Land/era5_land_evap_targetgrid.nc'
    ...
    regrid(grid=latlong_grid, infile=mintair_input, outfile=mintair_output)

   # *** If needed *** Concatenate files and save single NetCDF file
   ## filepath_in = "/path/to/EURO-CORDEX/RCP26/HIRHAM5/mintair_mon/" # Change path name to correct RCP and MODEL
   ## filenames = glob.glob(f"{filepath_in}mintair_HIRHAM5_RCP26_*.nc") # Change filenames to correct MODEL and RCP
   ## concatenate_input_files(files=filenames, file_conc=mintair_output)

    # Close datasets
    mintair_output.close()
    mintair_input.close()
