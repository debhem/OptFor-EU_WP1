###################################################################################################################################################
# Title: Script for Concatenating and Regridding Future Climate Scenarios of EURO-CORDEX Wind speed data

# Date: 20th January 2025

# Author: Dr Deborah Hemming and Dr Murk Memon, Met Office Hadley Centre, Met Office, UK

# Description: Code to concatenate (if needed) and regrid monthly mean future projection data from EURO-CORDEX regional climate models (RCM) for the variable "Wind speed"
...
#            - Downloaded as '10m Wind Speed' in m s-1, see https://cds.climate.copernicus.eu/datasets/projections-cordex-domains-single-levels?tab=overview
#            - With the naming format (for data in 5-year chunks) is "/path/to/EURO-CORDEX/RCP26/HIRHAM5/windsp_mon/windsp_europe_[MODEL]_[SCENARIO]_mon_2006_2100.nc" 
...
# Outputs: NetCDF time series files (2006-2100) for the variable "Wind speed" 
#          With the naming format "/path/to/EURO-CORDEX/RCP26/HIRHAM5/concat/windsp_europe_[MODEL]_[SCENARIO]_mon_2006_2100.nc" 
...
# *** If needed *** If data have been downloaded from CDS it will be in 5-year time chunks, these will need concatenating into a single NetCDF file
   ...
   ## files = /path/to/EURO-CORDEX/RCP85/HIRHAM5/windsp_mon/
...
def main():
   # Location of the input EURO-CORDEX HIRHAM5 wind speed data file/s
   ...
   windsp_input = '/path/to/EURO-CORDEX/RCP26/HIRHAM5/windsp_mon/windsp_mon.nc' # Change filenames to correct RCP and MODEL

   # Location of the processed, output EURO-CORDEX HIRHAM5 specific humidity NetCDF data file
   windsp_output = '/path/to/EURO-CORDEX/RCP26/HIRHAM5/concat/windsp_europe_HIRHAM5_RCP26_mon_2006_2100.nc'
   ...
   latlong_grid = '/path/to/ERA5_Land/era5_land_evap_targetgrid.nc' # Can be any ERA5_Land .nc file
    ...
    regrid(grid=latlong_grid, infile=windsp_input, outfile=windsp_output)

   # *** If needed *** Concatenate files and save single NetCDF file
   ## filepath_in = "/path/to/EURO-CORDEX/RCP26/HIRHAM5/windsp_mon/" # Change path name to correct RCP and MODEL
   ## filenames = glob.glob(f"{filepath_in}windsp_HIRHAM5_RCP26_*.nc") # Change filenames to correct MODEL and RCP
   ## concatenate_input_files(files=filenames, file_conc=windsp_output)
