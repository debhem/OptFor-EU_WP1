...
# Description: Code to concatenate (if needed) and regrid monthly mean future projection data from EURO-CORDEX regional climate models (RCM) for the variable "precipitation"
...
#         EURO-CORDEX RCM climate projection data for monthly mean precipitation
#            - Downloaded as 'Mean precipitation flux' in kg m-2 s-1, see https://cds.climate.copernicus.eu/datasets/projections-cordex-domains-single-levels?tab=overview
#            - With the naming format (for data in 5-year chunks) is "/path/to/EURO-CORDEX/RCP26/HIRHAM5/precip_mon/precip_europe_[MODEL]_[SCENARIO]_mon_2006_2100.nc" 
#                 - Or if time series are already concatenated the naming format is "/path/to/EURO-CORDEX/RCP26/HIRHAM5/concat/precip_europe_[MODEL]_[SCENARIO]_mon_2006_2100.nc" 
#            - For 2 RCM - HIRHAM5 and RACMO22E
#            - And 3 future scenarios - RCP26, RCP45, RCP85

# Outputs: NetCDF time series files (2006-2100) for the variable "precipitation" 
#          With the naming format "/path/to/EURO-CORDEX/RCP26/HIRHAM5/concat/precip_europe_[MODEL]_[SCENARIO]_mon_2006_2100.nc" 
...
# *** If needed *** If data have been downloaded from CDS it will be in 5-year time chunks, these will need concatenating into a single NetCDF file
   ...
   ## files = /path/to/EURO-CORDEX/RCP85/HIRHAM5/precip_mon/
...
def main():
   ...
   ## filenames = glob.glob(f"{filepath_in}precip_HIRHAM5_RCP26_*.nc") # Change filenames to correct MODEL and RCP
