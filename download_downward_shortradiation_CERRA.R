...
# Description: Automates the retrieval of CERRA reanalyses data for the variable "time-integrated surface direct shortwave radiation flux"
...
#         Existing files in the directory: "nc/cerra/solar_downward_directshort/"
#         CDS API key and user credentials

# Outputs: GRIB files for the variable "solar downward direct shortwave radiation flux"
#          Saved in the folder "nc/cerra/solar_downward_directshort/" with the naming 
#          Format "solar_downward_directshort_03h_[year].grib"
...
ani_desc <-
  list.files("nc/cerra/solar_downward_directshort/", pattern = "*.grib") |>  # List files in the specified directory matching `.grib`
...
for (i in 1:length(ani)) {  # Iterate over each remaining year in `ani`
  ...
  request <- list(
    ...
    "variable" = "time_integrated_surface_direct_short_wave_radiation_flux",  # Variable: direct shortwave radiation flux
    ...
    "target" = paste0("solar_downward_directshort_03h_", ani[i], ".grib")  # Name of the output file, including year
  )
  ...
  wf_request(
    ...
    path = "nc/cerra/solar_directdownward_short"  # Directory to save the output data files
  )
}
