# Script Description:
# This script automates the download of daily total precipitation data from the Copernicus Climate Data Store (CDS)  the Copernicus European Regional ReAnalysis (CERRA) dataset.
# It:
# It:
...
# 3. Constructs a data request for each year, specifying parameters such as variable (maximum air temperature), daily aggregation, and output format.
# 4. Sends the requests to the CDS API and saves the downloaded data files in the designated folder.
# The data downloaded is in GRIB format and is saved with filenames corresponding to the year (e.g., 'tmax_1984.grib').
